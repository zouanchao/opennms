...
@Mapper(uses = {NodeMapper.class, EventMapper.class, MemoMapper.class, RelatedAlarmMapper.class,
        DateToTimeMapper.class})
public interface AlarmMapper {
    AlarmMapper INSTANCE = Mappers.getMapper(AlarmMapper.class);

    @Mapping(source = "alarmType", target = "type")
    @Mapping(source = "logMsg", target = "logMessage") // Is this one needed?
    @Mapping(source = "operInstruct", target = "operatorInstructions")
    @Mapping(source = "reductionKeyMemo", target = "journalMemo")
    AlarmDocumentDTO onmsAlarmToAlarmDocumentDTO(OnmsAlarm alarm);
}
